            ...
            <form wire:submit="save" class="space-y-6">
                ...
                <div>
                    <label class="block text-sm font-medium text-white/80 mb-2">
                        Visualisasi Target <span class="text-white/40">(opsional)</span>
                    </label>
                    <div class="flex items-center gap-4">
                        @if ($image)
                        ...
                        @elseif ($isEditMode && $target && $target->image_path)
                            <!-- Preview for existing -->
                            <img src="{{ Storage::url($target->image_path) }}"
                                class="w-24 h-24 object-cover rounded-xl border border-white/10">
                        @else
                            <div
                                class="w-24 h-24 bg-white/5 border border-white/10 rounded-xl flex items-center justify-center text-white/20">
                        ...
                        <div>
                            <label
                                class="cursor-pointer inline-flex items-center gap-2 px-4 py-2 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl text-sm text-white transition-colors">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" />
                                </svg>
                                <span>Pilih Gambar Impian</span>
                                <input type="file" wire:model="image" class="hidden" accept="image/*"
                                    {{ $hasActiveTarget && !$isEditMode ? 'disabled' : '' }}>
                            </label>
                            <p class="text-xs text-white/40 mt-2">Maks. 2MB (JPG, PNG)</p>
                        </div>
                    </div>
                    @error('image')
